    ...
    return (
        <section className="registration-page">
            ...
            <div className="comandos-responsive-form-grid">
                ...
                <div className="registration-field"><ComandosSelectField id="incorporation-item" label={tr("Receiving item")} value={itemId} disabled={!receiving} options={(receiving?.items ?? []).map(item=>({value:String(item.id),label:`${item.itemName ?? `#${item.itemModelId}`} — ${tr("accepted")}: ${item.acceptedQuantity}`}))} onChange={value=>{setItemId(value);setSerialId("")}} /></div>
                ...
                <div className="registration-field"><ComandosSelectField id="incorporation-serial" label={tr("Serial")} value={serialId} disabled={!acceptedSerials.length} placeholder={acceptedSerials.length ? tr("Select") : tr("Not applicable")} options={acceptedSerials.map(serial=>({value:String(serial.id),label:`${serial.serialNumber}${incorporatedSerialIds.has(serial.id) ? ` — ${tr("incorporated")}` : ""}`,disabled:incorporatedSerialIds.has(serial.id)}))} onChange={setSerialId} /></div>
                <div className="registration-field"><ComandosSelectField id="incorporation-location" label={tr("Initial stock location")} value={locationId} options={locations.map(location=>({value:String(location.id),label:location.label}))} onChange={setLocationId} /></div>
