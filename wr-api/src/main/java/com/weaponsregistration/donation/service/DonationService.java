package com.weaponsregistration.donation.service;
...
import com.weaponsregistration.donation.model.*;
...
public class DonationService {
 ...
 @Transactional public DonationView finalize(FinalizeRequest request){
  ...
  var donation=new Donation();donation.organization=organization;donation.unit=unit;donation.donor=donor;donation.donee=donee;donation.organizationName=organization.name;donation.unitName=unit==null?null:unit.name;donation.donorName=donor.fullName;donation.doneeName=donee.fullName;donation.term=request.term().trim();donation.finalizedAt=LocalDateTime.now();var actor=audit.actor();donation.finalizedById=actor.id();donation.finalizedByLogin=actor.login();donation.requestId=request.requestId();donation.requestFingerprint=fingerprint;em.persist(donation);
  ...
  for(var line:request.items().stream().sorted(Comparator.comparing(i->i.assetId()!=null?"A"+i.assetId():"B"+i.balanceId())).toList()){
   ...
   if(line.assetId()!=null){var asset=locked(AssetItem.class,line.assetId());scope(asset.location,organization.id,request.unitId());if(!"AVAILABLE".equals(asset.status))conflict("Asset "+asset.assetCode+" is no longer available.");notExpired(asset.validUntil);if(item.quantity.compareTo(BigDecimal.ONE)!=0)bad("Individual assets require quantity 1.");item.asset=asset;item.model=asset.model;item.location=asset.location;item.stockCode=asset.assetCode;changes.add(Map.of("resource","inventory/assets","recordId",asset.id,"before",Map.of("status",asset.status),"after",Map.of("status","DONATED")));asset.status="DONATED";
