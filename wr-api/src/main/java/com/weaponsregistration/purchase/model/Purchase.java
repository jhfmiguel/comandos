...
@Table(name = "erp_purchase",
    uniqueConstraints = @UniqueConstraint(columnNames = {"buyer_organization_id", "purchase_number"}))
public class Purchase extends CoreEntity {
    @ManyToOne(optional = false)
    @JoinColumn(name = "buyer_organization_id", nullable = false)
    public Organization buyerOrganization;

    @ManyToOne
    @JoinColumn(name = "supplier_organization_id")
    public Organization supplierOrganization;

    @OneToOne
    @JoinColumn(name = "procurement_process_id")
    ...
    public List<PurchaseItem> items = new ArrayList<>();

    public void recalculateTotals() {
        subtotal = items.stream().map(PurchaseItem::calculateTotal)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
        total = subtotal
            .subtract(discount == null ? BigDecimal.ZERO : discount)
            .add(freight == null ? BigDecimal.ZERO : freight)
            .add(taxes == null ? BigDecimal.ZERO : taxes);
    }
}