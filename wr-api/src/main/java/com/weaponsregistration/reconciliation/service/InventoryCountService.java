...
import com.weaponsregistration.reconciliation.dto.InventoryCountContract.*;
...
public class InventoryCountService {
    ...
    @Transactional public CountView count(long id,SubmitRequest r){
        ...
        var stored=items(id);if(r==null||r.items()==null||r.items().size()!=stored.size())bad("A counted quantity is required for every inventory item.");
    ...
    @Transactional public CountView approve(long id){
        ...
        for(var item:items(id)){if(item.countedQuantity==null)conflict("The count is incomplete.");var diff=item.differenceQuantity;if(diff.signum()==0)continue;
            if(item.asset!=null){var a=locked(AssetItem.class,item.asset.id);if(diff.signum()<0){String before=a.status;a.status="BLOCKED";item.adjustmentMovement=movement(a,null,c.location,diff);changes.add(Map.of("assetId",a.id,"before",before,"after",a.status,"difference",decimal(diff)));}else conflict("Register unidentified individual assets before approving a surplus.");}
            else{var b=locked(StockBalance.class,item.balance.id);var lot=locked(StockLot.class,item.lot.id);if(diff.signum()<0&&b.available.add(diff).signum()<0)conflict("Release reserved or blocked quantity before approving this shortage.");b.available=b.available.add(diff);lot.availableQuantity=lot.availableQuantity.add(diff);if(b.available.signum()<0||lot.availableQuantity.signum()<0)conflict("Inventory adjustment would create negative stock.");item.adjustmentMovement=movement(null,lot,c.location,diff);changes.add(Map.of("balanceId",b.id,"difference",decimal(diff)));}}
        c.status=status("APPROVED");c.approvedAt=LocalDateTime.now();var actor=audit.actor();c.approvedById=actor.id();c.approvedByLogin=actor.login();em.flush();var value=view(c);audit.record("inventory-counts",c.id,"APPROVE",null,Map.of("inventoryCount",value,"adjustments",changes));return value;
    }

    @Transactional public CountView cancel(long id){var c=locked(InventoryCount.class,id);access.requireEntity("inventory-counts","CANCEL",c);if(Set.of("APPROVED","CANCELLED").contains(c.status.code))return view(c);c.status=status("CANCELLED");em.flush();var value=view(c);audit.record("inventory-counts",c.id,"CANCEL",null,value);return value;}
    ...
    private StockMovement movement(AssetItem a,StockLot l,StockLocation location,BigDecimal q){var m=new StockMovement();m.asset=a;m.lot=l;m.location=location;m.nature="INVENTORY_ADJUSTMENT";m.quantity=q;m.movedAt=LocalDateTime.now();em.persist(m);return m;}
