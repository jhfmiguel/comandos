...
export async function POST(request: Request) {
    if (!localRequest(request)) {
        return Response.json(
            {
                error:
                    "O controle do bot está disponível " +
    ...
    if (
        !body.command ||
        !["start", "pause", "resume", "stop"].includes(
            body.command
        )
    ) {
        return Response.json(
    ...
    if (body.command === "start") {
        if (
            !body.workstream ||
            !workstreams.has(body.workstream)
        ) {
            return Response.json(
        ...
        if (executionMode === "until") {
            if (!body.stopAfterTask) {
                return Response.json(
                    {
                        error:
                            "Selecione o requisito " +
                            "até o qual o bot deverá trabalhar."
                    },
                    { status: 400 }
                )
            }

            const targetExists = tasks.some(
                (task) =>
                    task.name === body.stopAfterTask
            )
            ...
            return Response.json(
                {
                    ...
                    message:
                        executionMode === "until"
                            ? "BOT iniciado até o requisito selecionado."
                            : "BOT iniciado em modo contínuo."
                },
                { status: 202 }
            )
        } catch {
